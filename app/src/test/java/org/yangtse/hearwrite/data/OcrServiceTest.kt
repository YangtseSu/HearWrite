...
class OcrServiceTest {
    ...
    fun inferOcrLang_followsTheMajorityOfSpeakableHeadwords() {
        ...
        assertEquals(OcrLang.CHINESE, inferOcrLang(listOf("月亮", "生日", "school").map(::parseWordLine)))
        // 2 汉字 against 1 English headword: Chinese — the columns of an
        // English row never count for it.
        assertEquals(OcrLang.CHINESE, inferOcrLang(listOf("apple | n. | 苹果", "月", "生日").map(::parseWordLine)))
        // English columns never count as Chinese: the headword decides.
        assertEquals(OcrLang.ENGLISH, inferOcrLang(listOf("apple | n. | 苹果", "banana | n. | 香蕉").map(::parseWordLine)))

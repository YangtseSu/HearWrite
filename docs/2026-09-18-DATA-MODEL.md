...
| 界面 | 显示 | 实测 |
|---|---|---|
| **拨盘**（听写中） | 音标**与词性合成同一行**：`/ˈæpəl/ n.` | 204 dp 盘、单行词时内容框宽 **147.5 dp = 9.8 units**；美式音标宽度中位 **4.0 u（60 dp）**、p99 7.5 u、最大 10.0 u。**上面只量了音标**：把词性尾巴一起算进合成行后，已上架英文行里有 **125 行**超过 9.83 u 的框（最宽 `仁爱版初中/七上 Unit 4 探究.txt` 的 `/bɪˈfɔː(r)/ prep. & conj.`，12.5 u ≈ 187 dp）——修法是 `dialFit` 现在**测量提示文本**，一行放不下就置 `needsDetail`，由「展开全部 → 详情卡」兜住 |

...
- ✅ **冷启动与内存预算** — 已决策并复测。词典保持资产惰性加载（`LexiconRepository` 首次查询时才解析 6.5 MB → 约 13 MB 常驻、解析峰值 ~54 MB，不在启动路径上；实测见 `AGENTS.md`），Phase 10 的真机 release 稳态 173 ms 已低于 500 ms 阈值，故不迁 `Room.createFromAsset`（记录见 `2026-09-04-PHASES.md`）。**2026-09-18 真机复测（v0.8.0 release，API 36）**：`am start -W` 首启 326 ms（odex 预热），随后 5 次 force-stop 冷启 **180 / 181 / 183 / 205 / 227 ms**（中位 183），全部远低于 500 ms；同一构建在软件渲染模拟器上是 700–727 ms、同机系统设置基线 243–292 ms，故模拟器数字不作阈值判据。流程：`adb install -r` release 包 → `svc power stayon true` → 逐次 `am force-stop` + `am start -W`。

- ✅ **大屏 / 横屏 / 折叠屏适配** — 已落地（2026-09-16，UI 审计 C6）：`ui/Adaptive.kt` 给出窗口尺寸接缝（`currentWindowAdaptiveInfoV2().windowSizeClass`、`WIDTH_DP_MEDIUM`）与 720 dp 阅读宽度上限（`Modifier.contentWidth`，29 处调用点覆盖各页面与两条底栏，另有 1 处定义；内容居中而不再拉满）；听写舞台由 `domain/DialStage.kt` 从实测框解算 STACKED / BESIDE，双栏以 WindowSizeClass 为门（`ui/DictationScreen.kt` 的 `isWindowAtLeast(WIDTH_DP_MEDIUM)` 与传给 `dialStageGeometry` 的 `twoColumnsAllowed`）；词库页用 `GridCells.Adaptive(180.dp)`；旋转敏感状态进 `rememberSaveable`，manifest 不锁方向。**未采样（如实记录）**：折叠屏铰链姿态与分屏（freeform）——可用硬件里没有这两种形态（审计 §未能闭环项）。
...
- **词库质检脚本（`scripts/check-assets.py`）+ CI** — ✅ 已落地（见 [`../ROADMAP.md`](../ROADMAP.md) 条目 16）：651 表 / 21769 行，校验每行可解析、无重复词、拼音格式合法、组词含本字、多字中文行不带列、生成资产（`lexicon-en.json` / `lexicon-hanzi.json` / `compounds.json`）结构完整；CI 先于 Gradle 执行。词库名/分类名的排序漂移仍由 `LabelOrderTest` 守住。

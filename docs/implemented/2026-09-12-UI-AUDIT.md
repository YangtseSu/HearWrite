...
**验收证据（2026-09-12，模拟器，无截图，全部为 `uiautomator dump` 的属性/坐标断言）**：
...
**验收证据（2026-09-14，模拟器，`uiautomator dump` 属性断言）**：
...
**验收证据（2026-09-15，模拟器，`uiautomator dump` 属性/坐标断言）**：
...
**验收证据（2026-09-15，模拟器 1080×2424 @420dpi，`uiautomator dump` 属性/坐标断言）**：
...
**验收证据（2026-09-16，模拟器 1080×2424 @420dpi，`uiautomator dump` 属性/坐标断言 + 程序化像素取样）**：
...
**验收证据（2026-09-16，模拟器 1080×2424 @420dpi，`uiautomator dump` 属性/坐标断言 + 程序化像素取样 + `dumpsys vibrator_manager`）**：
...
| 双击可能叠栈 | C1c 已给四个顶层路由补齐；本阶段发现**同类缺陷仍在带参路由上**（`library_list/{category}`、`library_preview/{category}/{label}` 由分类卡/词表行双击可达，同样会压两层）。新增的结束页入口也不再自写 `navigate`，统一走单顶辅助函数 `openTop`——全文件 **11 处 push 经它**，另 2 处直连 `navigate`（`startSession` 的听写页与结束页的 `STATS` 入口）各自也带 `launchSingleTop`，"新增目的地忘记加"这个类别被消灭 |
...
**验收证据（2026-09-16，模拟器 1080×2424 @420dpi，`uiautomator dump` 属性/坐标断言）**：
...
**未能闭环项（如实记录）**：
- **"双击叠栈"的设备判据弱**：`adb shell input tap` 两次同坐标的间隔受 shell 往返限制（≈0.6 s），比真实连点慢，只采样到"双击后一次返回回到列表"这一条；`launchSingleTop` 本身由 `openTop` 单点强制，11 处调用点无一绕过，另两处直连 `navigate` 也都自带该 flag（源码层可穷举）。
...
**验收证据（2026-09-16，模拟器，`uiautomator dump` 属性/坐标断言 + 程序化像素取样）**：
...
**未改（如实记录）**：`CHANGELOG.md` 0.6.0 一节里的「多选词库 / 抽词听写」保持原文——它是已发版的 Release 正文（GitHub Release body 由该节生成），改写已发布的说明是改写历史，与 `ERRATA.md` 的口径一致；`docs/implemented/2026-09-12-UI-AUDIT.md` 自身的发现文本按基线原样保留。

**验收证据（2026-09-17，模拟器 1080×2424 @420dpi，`uiautomator dump` 文本 / `bounds` 断言）**：
...
**验收证据（2026-09-18，模拟器 1080×2424 @420dpi；手段 = `screenrecord` → `ffmpeg -fps_mode passthrough` 抽帧 → PIL 程序化取样，脚本为一次性 /tmp 工具不入库）**：
...
**验收证据（2026-09-18，模拟器 1080×2424 @420dpi；两个采样点之间系统 night mode 恒为 `no`，只切应用 `主题`）**：
...
**验收证据（2026-09-18，模拟器；整屏精确色值直方图，一次性脚本 `/tmp/palette.py`，不入库）**：

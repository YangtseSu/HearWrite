...
- **多选模式**：新增 `data/LibrarySelectionStore.kt` ——进程级选中状态（跨页面、跨分类），词库浏览页与分类页顶栏「多选」图标进入，底栏「抽词听写」发起；「完成」/「退出多选」/系统返回退出多选并清空选中，**回到首页也复位**（`HomeScreen` 进入时清）——任何一次进入多选都是干净状态，隐藏的旧选中不会在下次访问冒出来。底栏常显「已选 N 个词表」，未选时入口禁用。列表行选中态用 Checkbox（「选择词表 X」TalkBack 描述），全库搜索的「词表 / 词条」结果行同样可勾选。
...
现状：内置英文词库来自 alice 项目拷贝，中文词库为手动录入，两者均只覆盖部分内容。当前共 **469 份词表 / 19114 行**：
...
|人教版小学语文|158|1352|一上 + 二上（识字表/写字表/词语表）|
|义务教育语文课程标准|3|3800|基本字表 300 + 常用字表 2500/1000，已完整收录|

**已完成（2026-09-10）**：

- `docs/WORDLIST.md` — 词库数据契约与贡献指南（格式、命名、校验、授权、导入流程）。
- `scripts/check-assets.py` — 全库质检（结构错误退出码 1；`*`/省略号类词头等给 warning），已接入 CI，先于 Gradle 步骤执行。当前 469 表 / 19114 行 0 error。
...
1. **人教版小学语文 其余册次**（一下/二下 + 三~六上下，约 270 表 ≈ 7300 行）——一上已于 2026-09-13 补齐，剩余册次仍是最大缺口，且有一二年级用户。
...
- **词库质检脚本（`scripts/check-assets.py`）+ CI** — ✅ 已落地（见正文 16）：469 表 / 19114 行，校验每行可解析、无重复词、拼音格式合法、组词含本字、多字中文行不带列、生成资产（`ecdict-meta.json` / `hanzi-meta.json` / `compounds.json`）结构完整；CI 先于 Gradle 执行。词库名/分类名的排序漂移仍由 `LabelOrderTest` 守住。

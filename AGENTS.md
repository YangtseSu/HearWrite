...
- `ui/` — Compose screens (`HomeScreen`, `DictationScreen`, `SettingsScreen` + sub-pages, `LibraryScreen`/`LibraryListsScreen`/`LibraryPreviewScreen`, `StatsScreen`, `OcrScanSheet`), reusable components (`CountdownRing`, sheet hosts in `HomeDrawers`, crop overlay, `ListRows`), `theme/`. Material 3; light + dark themes; Chinese strings inline.
...
2. **Microsoft Edge Read-Aloud** (微软 Edge source, keyless): the neural voices behind Edge 朗读. The wire protocol is **vendored**, not reimplemented: `app/src/main/java/io/edge/EdgeTts.kt` is the standalone Kotlin client (okhttp/okio + coroutines + `org.json`, no Android API) that speaks the service's current protocol — `wss://speech.platform.bing.com/consumer/speech/synthesize/readaloud/edge/v1` with `TrustedClientToken`, `Sec-MS-GEC` (SHA-256 of `<%.0f winTicks><token>`, winTicks = unix + 11644473600 floored to 300 s epoch windows ×10⁷, uppercase hex), `Sec-MS-GEC-Version`, full Edge/Chromium headers (UA, `Accept-Encoding: gzip, deflate, br, zstd`, `Pragma`/`Cache-Control`, `Origin: chrome-extension://jdiccldimpdaibmpdkjnbmckianbfold`, `Sec-WebSocket-Version: 13`, fresh 32-hex `muid` cookie per connection). One WebSocket turn per clip: `Path:speech.config` JSON frame (outputFormat `audio-24khz-48kbitrate-mono-mp3`), then a `Path:ssml` frame (voice per language, prosody rate/pitch/volume); binary frames = 2-byte big-endian header length + `Path:audio`/`Content-Type:audio/mpeg` header block (its trailing `\r\n` included in the length) with the MP3 chunk starting immediately after it, `Path:turn.end` ends the turn; long text is split at 4096-byte chunks, each on its own connection. **音色选择**: the 发音来源 page (only when 微软 Edge is selected) offers one dropdown per language — 中文音色 (zh-CN) and 英文音色 (en-US) — over the curated catalog (`data/EdgeTts.kt` `EDGE_VOICE_CATALOG` — Mainland zh-CN voices for Chinese, en-US for English, zh-HK/zh-TW and dialects excluded because they cannot serve simplified 普通话 lists), styled like the TTS API form fields, each with a 试听 button playing the current selection (synthesizes the sample without selecting). No separate 默认 entry: the catalog contains the built-in defaults 晓晓/Aria and picking one persists as blank (= built-in default). Per-language selections persist under DataStore (`edge_voice_zh` / `edge_voice_en`; blank = built-in default 晓晓/Aria) and live-follow into playback; the clip cache keys bind voice + rate so a switch regenerates instead of replaying stale audio. The app's `data/EdgeTts.kt` is only a disk-cache + per-clip single-flight wrapper over the vendored client — it decides the voice, warms `cacheDir/tts/edge-…` in the background and serves ready clips; playback never blocks on a download. 403-with-`Date`-header clock-skew correction is handled inside the vendored client. ⚠️ **Maintenance ritual**: Microsoft has broken this endpoint repeatedly (2023 Sec-MS-GEC era; 2025-12 MUID/UA/chunk changes) — when it breaks, sync `io/edge/EdgeTts.kt` with the standalone reference ([edge-tts-kotlin](https://github.com/YangtseSu/edge-tts-kotlin); upstream rany2/edge-tts is **LGPLv3** — the vendored file carries the provenance notice) and re-verify on a device (the app's own reimplementation was retired 2026-09 because the service silently dropped its connections while the vendored client worked).
3. **System TTS** (fallback, always available): `android.speech.tts`, locale `en-US`/`zh-CN` by entry kind, speech rate from settings (0.5–1.5, default 0.9). **音色 selection** (设置 → 系统语音, persisted `system_voice_zh`/`system_voice_en` DataStore keys): the engine's `getVoices()` set is enumerated once after a successful init and cached; per-language voice keys (`Voice.name`, blank = engine default) live-follow into playback like the Edge voices, applied via `setVoice` **after** `setLanguage` (setLanguage resets the voice to the locale default — the order decides whether the chosen voice actually speaks). Android gives no display name/gender for a `Voice` — the picker labels come from the pure helpers in `data/SystemSpeaker.kt` (`systemVoicesFor`/`systemVoiceInfos`, JVM unit-tested on the `SystemEngineVoice` model): a readable engine name (`com.apple.voice…Samantha` → `Samantha`) is kept, a meaningless generated id (`cmn-cn-x-ssa-local`) falls back to 中文男声1 / 中文女声1 / 中文语音1-style naming (英文… for English; gender from the engine's `#female`/`#male` markers). Voices are filtered to simplified-Mainland (zh/cmn + CN) and US-English only — zh-TW/zh-HK/yue and en-GB cannot serve the textbook lists. Wrap async init in `suspendCoroutine` resumed from `onInit`; resume the pending continuation from **both** `onDone` and `onError` of `UtteranceProgressListener` (missing `onError` = permanent hang) **or** a watchdog (`max(4000, text.length × 250)` ms) that assumes success — a mute utterance must never freeze playback; play cached MP3 clips with `MediaPlayer` guarded by a completion listener **plus a 10 s watchdog** (Media3 only if a concrete need appears).
...
- **Room**: `wrong_words` (word, addedAt, errorCount, lastWrongAt, sourceLabel — the speakable-headword key; `errorCount` counts dictation **runs**, not presses), `history` (user lists only — id, text, enriched text, createdAt; **cap 50**, drop oldest; favorited rows exempt), `favorites` (entry ids: `default_*` or history ids), `sessions` (Roadmap #3 听写统计: one row per **completed** run — startedAt, sourceLabel, totalWords, wrongCount, durationSec, kind = `dictation`/`review`; a stopped/abandoned run records nothing). Wrong words and favorites are keyed by the speakable headword / entry id (`default_*` built-in ids, history ids for user lists). Aggregation is pure (`domain/Stats.kt` — `summarize`/`dailyStats`/`streakDays`, zone + today injected), rendered by `ui/StatsScreen.kt` (首页 更多 → 听写统计) with no chart library. **Schema is exported** (`exportSchema = true`, ksp `room.schemaLocation` → `app/schemas/`) and the JSON is committed — bump `version` + add a migration whenever entities change; never change entities silently.
- **DataStore Preferences**: word-input draft (debounced 500 ms; the library preview's 载入草稿 feeds it via a one-shot import bus, `HearWriteApplication.requestDraftImport`), speech rate, interval sec, auto-next, read-translation, sound effects on/off, TTS source (`youdao|edge|system|custom`), one provider config JSON per preset (map keyed by preset id; legacy single-blob keys are migrated away on write) for TTS and OCR alike, theme (light/dark/system). **BYOK provider API keys are sealed at rest** before the config maps are written — AES-256-GCM via the Android Keystore (`data/KeystoreCipher.kt`: key generated in-chip, never exported; stored shape `v1.<iv-b64>.<ct-b64>`). Key generation first requests a **StrongBox**-backed key; a device without StrongBox throws `StrongBoxUnavailableException` at `generateKey()`, which is caught and retried **without** StrongBox — only a total keystore failure degrades to plaintext (best-effort at-rest protection), and the save flow then reports it to the UI (`ttsKeySealed`/`ocrKeySealed` → a one-shot hub toast; never a silent plaintext save). Legacy plaintext keys (before 0.3.2) are not our shape → read through untouched and sealed on the next save; a sealed value that can no longer be decrypted (keystore wiped on restore/transfer) resolves to "" so the user re-enters the key rather than the app sending garbage. The JVM tests drive a fake cipher through the `SecretCipher` seam (`ProviderConfigSealTest`). `data_extraction_rules.xml` excludes `datastore/` and the database from cloud backup and device transfer — sealed keys never leave the device.
...
- Unit tests (JUnit4 + `kotlinx-coroutines-test`) cover the `domain/` behavioral contract — line parsing (1/3 columns, fullwidth pipe, `you're = you are`), POS normalization, CJK detection, `cjkWordSpeech` (tier order, polyphone filtering with the `朝|zhāo → 朝阳` case, learned-first), `speakableMeaning` (POS strip + sense split + width cap), `compareLabels` ordering (against the `library-label-order.json` parity fixture), `DictationEngine`, and the 听写统计 aggregation (`Stats.kt` — day bucketing by local day, window zero-fill, streak rules) — plus the `data/` pure logic: Youdao URL/cache-file forms, TTS/OCR provider-config codecs and wire bodies (clip hash, `speech`/`chat` bodies), OCR reply extractors and fence stripping, the session record's clamping/kind-degradation rules. Engine tests use a fake `Speaker` and `runTest` virtual time: phase order, no-retry on speak failure, cancel leaves no stray speaks, auto-next hold, generation races, and ★ live interval change verified via `advanceTimeBy`.
- Assets/dict data are **read-only fixtures** — tests may load from `app/src/main/assets/` but never modify it.
- `app/src/androidTest` (Room): real-SQLite migration tests (`MigrationTestHelper` — v1→v2 wrong words, v2→v3 sessions, each diffing the migrated schema against the committed JSON) plus DAO behavior the JVM fakes cannot reach (recordMark upsert counts, favorited-history cap exemption). Any new migration MUST get one; `connectedDebugAndroidTest` runs them on CI's API 36 emulator.

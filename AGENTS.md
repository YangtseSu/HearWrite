...
`Speaker` contract: `suspend fun speak(text, lang): Boolean` + `stop()` — returns `false` on failure, never throws into the caller. The 组词 phrase (`"月亮的月"`) **always** speaks via system zh-CN TTS — Youdao dict voice cannot serve sentences, and the phrase pass stays system-pinned even under the Edge source (phrases are never prefetched, so a ready-clip chain would fall straight through anyway). Any failure falls through to the next link; dictation never blocks on a download or an outage.

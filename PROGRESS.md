...
- [ ] `DictationEngine` (coroutine state machine): speak1 → 700 ms → speakMeaning (optional) → speak2 → interval countdown → next; pause/resume; live interval changes; stop on exit. Settings defaults: interval 7 s (1–10, step 0.5), rate 0.9 (0.5–1.5), auto-next on.
- [ ] System `TextToSpeech` wrapper (`en-US`/`zh-CN` by entry kind) + DataStore-backed settings.
- [ ] Home: paste word list (draft persisted, 500 ms debounce) → start dictation; Dictation screen: countdown ring, show/hide word, POS/meaning hint, 标记错词.

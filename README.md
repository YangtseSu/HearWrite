...
- **内置词库**：中考1600、高考3500、初中2182、人教版/外研版/闽教版/仁爱版等 10 套教材，外加课标字表（基本字表 300 / 常用字表 2500 + 1000），共 651 份词表，支持按词表名或单词搜索；词表预览页可直接开始听写（同样支持随机顺序与起始序号），或一键载入草稿继续编辑。
- **汉字提示自动补全**：粘贴或拍照得到的生字表只有一个字也没关系——应用会用内置数据补上拼音与组词（组词朗读同样可用），与英文词表的离线释义补全走同一条路。
- **灵活导入**：粘贴输入（每行一个词）；拍照识词——拍课本词表可先框选识别区域，AI 识别英文词表或汉字生字/词语（识别结果可编辑后再开始）。
- **离线词典补全**：英文词表自动补全**音标**（英式 / 美式两套，仁爱教材印过的词用课本上的记号）、词性与中文释义（内置 ECDICT 离线词典），单个汉字自动补全拼音与组词——都不联网；列表行与听写拨盘都会显示音标，拨盘把音标与词性并成一行，不占额外高度。
...
- **多选词表 / 抽词听写**：在词库里跨分类勾选多份词表，显示合计词数，随机抽 X 个词听写（X 可填，默认全部即全量随机）；跨表重复词自动合并，被标记的错词仍能回查原词表。
- **拍照批改**：听写结束后拍下学生写在纸上的答案，应用逐词比对本次词表并给出成绩（正确 / 错词 / 漏答 / 多余），拼写接近、空白与多余的行标记「存疑」供人工核对；确认后才记入错词本，与手动标记的错词同样累计次数与来源。
...
- **播放控制**：语速、间隔（1–10 秒）、自动播报下一词、朗读英文释义，听写中可实时调整；听写中屏幕常亮（听写、暂停、结束页都不熄屏，离开即恢复系统熄屏）；倒计时最后一秒滴答声、完成提示音、标记错词震动。
- **深色 / 浅色主题**（可跟随系统）。

## 下载安装

- 从 GitHub [Releases 最新版](https://github.com/YangtseSu/HearWrite/releases/latest) 页面下载 APK 安装（页面顶部 Release 资产即当前最新版；文件名形如 `HearWrite-0.5.0.apk`，随版本变化）。需允许安装未知来源应用。
- 也可以自行构建，见下「从源码构建」。
...
1. **导入词表**（首页）：直接粘贴；点「英文示例」「汉字示例」体验格式；或点「拍照识词」拍课本词表（可先框选识别区域、选英文或汉字识别）；或从「更多」菜单进「词库」选一份内置词表——也可以点右上角「多选」跨分类勾选多份词表，再点「抽词听写」随机抽 X 个词。
2. **听写**：点「开始听写」。每词朗读两遍，词盘中的词语默认隐藏（学生先默写），点按显示；最后一秒倒计时结束自动播下一词；写错了点「标记错词」。
3. **批改**：学生写完，在结束页点「拍照批改」拍下答案纸（可框选区域），应用比对本次词表并逐词判定；核对列表里标「存疑」的行、按需勾选后点「确认错词」，错词就进错词本。
4. **复习**：结束后在成绩卡点「复习错词」只重听错词；「导出错词」把错词复制成词表，粘贴回首页即可再练；首页「更多」菜单的错词本也能随时复习。成绩卡的「听写统计」直接打开本次刚写入的记录（首页「更多 → 听写统计」同样可进）。
...
| 发音来源 | 有道词典（默认，需联网；失败自动回退系统语音）／微软 Edge（免费、无需 API Key，可选中文/英文音色；失败自动降级）／系统语音（完全离线）／TTS API（OpenAI 兼容，自备 API Key；预设小米 MiMo（免费）与自定义，各服务商配置独立保存） |
| 提示音 | 倒计时滴答与完成提示音开关 |
| 清空发音缓存 | 删除已下载的发音文件，之后需要时自动重新下载 |
| 拍照识词 / 拍照批改 | 识别服务：OpenAI 兼容视觉接口，需自备 API Key；默认预设智谱 `glm-4v-flash`（免费），另有 OpenCode Zen、Vercel、Command Code、OpenRouter 预设，或自定义任意接口（本机回环接口亦可，如 Ollama 需 `adb reverse tcp:11434`） |

**API Key 只保存在本机**（应用私有存储，经 Android Keystore 加密后写入；个别设备缺少安全存储硬件时会提示并以明文降级保存），仅用于向对应服务商发请求；本应用没有后端服务器，不上传任何词表或听写记录。AI 识图可能存在误差，请核对识别结果（拍照批改的判定同样仅供参考，确认后才记入错词本）。
